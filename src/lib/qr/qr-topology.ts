...
import type { CapStyle, ErrorCorrectionLevel, ModuleStyle } from './generate';
import { isConnectionModeConfigurable } from './generate';
...
export function edgeKey(r1: number, c1: number, r2: number, c2: number): Edge {
	return `${r1},${c1}-${r2},${c2}`;
}
 ...
 * Returns [TL, TR, BR, BL].
 */
export function exposedCorners(adj: Adj): [boolean, boolean, boolean, boolean] {
	return [
		!adj.top && !adj.left,
		!adj.top && !adj.right,
